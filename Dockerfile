# Dockerfile for testing Psiphon only

# 1️⃣ Use a minimal Linux base image
FROM debian:bullseye-slim

# 2️⃣ Create and switch to working directory
WORKDIR /app

# 3️⃣ Copy your Psiphon config into the image
#    (Place psiphon.config alongside this Dockerfile)
COPY psiphon.config ./

# 4️⃣ Install wget and CA certificates, download Psiphon binary, mark executable
RUN apt-get update && \
...
# 5️⃣ (Optional) Expose local proxy ports defined in your config
EXPOSE 1080 8081

# 6️⃣ Run Psiphon with your configuration
ENTRYPOINT ["./psiphon", "--config", "psiphon.config"]
